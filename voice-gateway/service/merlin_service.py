...
# ── INPUT DEVICE OVERRIDE (2026-08-17 live rescue) ───────────────────────────
# The Babyface input path physically degraded mid-session (measured: the
# same claps at peak 0.110 → 0.018 within an hour; whisper garble on every
# turn). MERLIN_INPUT_DEVICE_NAME selects any other CoreAudio input by name
# substring (e.g. the iPhone Continuity mic, measured 6× stronger) WITHOUT
# touching the output device (TTS stays on the Babyface). Applied once at
# import, before any stream opens; a missing/vanished device logs and
# falls back to the system default rather than crashing.
def _apply_input_device_override() -> None:
    name_sub = os.environ.get("MERLIN_INPUT_DEVICE_NAME", "").strip()
    if not name_sub:
        return
    try:
        ...
        for i, d in enumerate(devs):
            if d.get("max_input_channels", 0) > 0 and name_sub.lower() in str(d.get("name", "")).lower():
                cur = _sd.default.device
                out_dev = cur[1] if isinstance(cur, (list, tuple)) else cur
                _sd.default.device = (i, out_dev)
                logging.getLogger("merlin.service").info(
                    "INPUT_DEVICE_OVERRIDE name=%r -> device %d (%s, %dch); output unchanged (%s)",
                    name_sub, i, d.get("name"), d.get("max_input_channels"), out_dev)
                return
        logging.getLogger("merlin.service").warning(
            "INPUT_DEVICE_OVERRIDE name=%r matched no input device — using system default", name_sub)
    except Exception as _exc:
        logging.getLogger("merlin.service").warning("INPUT_DEVICE_OVERRIDE failed: %s", _exc)

_apply_input_device_override()
...
# Merlin Product Recovery (2026-08-17): 0.8s trailing silence cut natural
# sentences at mid-sentence pauses (live operator complaint: "cuts the
# sentence prematurely"). Natural Hebrew speech pauses commonly run
# 0.6–1.2s; 1.4s keeps one thought = one turn at the cost of ~0.6s extra
# latency at true end-of-utterance. The initial-silence budget and
# FALSE_START guard are unchanged — this only affects the trailing edge.
SILENCE_S    = 1.4     # trailing silence that ends an utterance
MAX_RECORD_S = 30
...
# 2026-08-17 21:05 LIVE PRODUCT FAILURE ("הוא אטום"): 8s of silence dropped
# the session to standby SILENTLY between the user's questions — at natural
# conversation pacing (thinking, moving around) he was constantly speaking
# to a sleeping service with no working voice wake at current mic levels.
# 60s keeps the conversation open across natural pauses; VAD still gates
# capture, so an open-but-quiet window costs nothing.
CONVERSATION_TIMEOUT = 60.0   # seconds of post-response silence → standby
...
async def record_utterance(
    ...
    # 2026-08-17 21:15 CRASH-LOOP FIX (same class as wake_trigger's):
    # device-list/default reprs RAISE when the iPhone Continuity mic
    # appears/vanishes mid-enumeration — diagnostics must never be fatal.
    try:
        logger.info("[rec] sd.query_devices():\n%s", str(sd.query_devices()).replace("%", "%%"))
        logger.info("[rec] sd.default.device     = %s", str(sd.default.device))
        logger.info("[rec] sd.default.samplerate = %s", str(sd.default.samplerate))
        logger.info("[rec] sd.default.channels   = %s", str(sd.default.channels))
    except Exception:
        logger.info("[rec] device diagnostics unavailable (formatting)")
...
async def run_conversation_session(
    ...
    _from_barge = False
    # Post-TTS echo window (turn_guard.POST_TTS_ECHO_MIN_CONFIDENCE): True
    # ONLY for the first mic capture after Merlin's own playback — the one
    # capture where low-confidence garble is far more likely playback bleed
    # than user speech. One-shot; cleared at the next STT evaluation.
    _post_tts_echo_risk = False
    ...
    while True:
        ...
        if control_state is not None:
            ...
            if injected is not None:
                ...
                _post_tts_echo_risk = not interrupted   # Merlin just spoke — arm the echo window
        ...
        if not audio:
            ...
            # Audible sleep cue (2026-08-17): the user must HEAR that the
            # session closed — a silent drop to standby reads as "he's
            # ignoring me". Same chime used on wake, so the vocabulary is
            # one sound = state change.
            try:
                player.chime()
            except Exception:
                pass
        ...
        _echo_risk_now, _post_tts_echo_risk = _post_tts_echo_risk, False   # one-shot
        accepted, reject_reason = evaluate_transcription_confidence(transcription, post_tts_echo_risk=_echo_risk_now)
        if not accepted:
...
async def main() -> None:
    ...
    # Bind a profile_id to the background session so adapter.get_profile_id()
    # can resolve a subject (Essence context fetch, PHILOS observation gate,
    # PHILOS orientation interview). Without this, register_session() is never
    # called anywhere on this code path and every profile-dependent feature is
    # permanently inert. MERLIN_BG_PROFILE_ID unset -> unbound, same as before
    # this change (no behavior change unless explicitly configured).
    if hasattr(adapter, "register_session"):
        _bg_profile_id = os.environ.get("MERLIN_BG_PROFILE_ID") or None
        if _bg_profile_id:
            from app.adapters.merlin import VoiceSessionContext
            adapter.register_session(VoiceSessionContext(session_id=session_id, profile_id=_bg_profile_id))
            logger.info("Bound a profile_id to background session_id=%r", session_id)  # value itself never logged
    ...
    while True:
        try:
            control_state.set_state("IDLE")
            # INJECTION RACE FIX (2026-08-17): a SEND-AS-USER posted in the
            # gap between session end and trigger re-arm had its manual_wake
            # wiped by event.clear(), leaving the text queued until the next
            # physical wake. If text is already waiting, skip the wait —
            # the session loop pops it on its first iteration.
            if control_state.has_pending_injection():
                pending, wake_source = [], "manual"
                logger.info("WAKE_FROM_QUEUED_INJECTION — bypassing trigger.wait()")
            else:
