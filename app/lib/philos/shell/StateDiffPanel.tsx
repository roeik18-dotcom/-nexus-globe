...
export default function StateDiffPanel({ selected }: { selected: FoundContext }) {
  ...
  const wrap = { fontSize: 13, color: "#cfe0f5", background: "rgba(90,120,180,0.06)", border: "1px solid #2a3f66", borderRadius: 8, padding: "10px 14px", marginBottom: 12 } as const;

  if (selected.priorState === undefined) {
    return (
      <div dir="rtl" style={wrap}>
        <div style={{ fontSize: 12, letterSpacing: 1.5, color: "#5aa6ff", marginBottom: 4 }}>STATE BEFORE → AFTER</div>
        <div style={{ color: "#6c86b5" }}>
  ...
  return (
    <div dir="rtl" style={wrap}>
      <div style={{ fontSize: 12, letterSpacing: 1.5, color: "#5aa6ff", marginBottom: 4 }}>STATE BEFORE → AFTER</div>
      <div style={{ fontSize: 12, color: "#7b8ca6", marginBottom: 8 }}>
        entity id: {selected.matched_id} · dimension: {selected.domain}{selected.frame ? `/${selected.frame}` : ""}
      </div>
      <div style={{ display: "flex", alignItems: "stretch", gap: 10, flexWrap: "wrap" }}>
        <div style={box}>
          <div style={{ fontSize: 12, letterSpacing: 1, color: "#6c86b5" }}>BEFORE · {prior.observed_at.slice(0, 16).replace("T", " ")}</div>
          <div style={{ fontSize: 13, marginTop: 4 }}>level {prior.level} · stability {prior.stability}</div>
        </div>
        <div style={box}>
          <div style={{ fontSize: 12, letterSpacing: 1, color: "#6c86b5" }}>AFTER · {selected.timestamp?.slice(0, 16).replace("T", " ") ?? ""}</div>
        ...
        {delta ? (
          <div style={{ ...box, borderColor: "#5a4a2a" }}>
            <div style={{ fontSize: 12, letterSpacing: 1, color: "#fbbf24" }}>Δ CHANGE</div>
            <div style={{ fontSize: 13, marginTop: 4 }}>
      ...
      <div style={{ fontSize: 12, color: "#6c86b5", marginTop: 8 }}>
