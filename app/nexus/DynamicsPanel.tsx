...
export default function DynamicsPanel({ selected, allNodes, proofTrustMap, onTransition }: Props) {
  ...
  const S = {
    ...
    head:    { fontSize: 12, letterSpacing: 2, color: "#1e4060", textTransform: "uppercase" as const, marginBottom: 6 } as React.CSSProperties,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ padding: "10px 14px 6px", borderBottom: "1px solid #0a2a4a", flexShrink: 0 }}>
        <div style={{ fontSize: 12, letterSpacing: 3, color: "#38bdf8", fontWeight: 700, marginBottom: 5 }}>
          NEXUS DYNAMICS
        </div>
        {selected && (
          <div style={{ fontSize: 12, color: fc }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px" }}>
        ...
        {tab === "evolution" && (
          <div>
            {!selected && <div style={{ fontSize: 12, color: "#1e4060", textAlign: "center", padding: 16 }}>בחר node לראות נתיב אבולוציה</div>}
            {selected && evo && (
              <>
                <div style={S.section}>
                  ...
                  {[
                    ...
                    <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "flex-start" }}>
                      ...
                      <div>
                        <div style={{ fontSize: 12, color: "#1e4060", letterSpacing: 1, marginBottom: 2 }}>{step.label}</div>
                        {step.node && (
                        ...
                        <div style={{ fontSize: 12, color: "#8bb8cc" }}>{step.text}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={S.card(fc)}>
                  ...
                  <div style={{ fontSize: 13, color: fc, fontWeight: 700 }}>{trust}/100</div>
                  ...
                  <div style={{ fontSize: 12, color: "#1e4060", marginTop: 4 }}>
        ...
        {tab === "timelines" && (
          <div>
            {!selected && <div style={{ fontSize: 12, color: "#1e4060", textAlign: "center", padding: 16 }}>בחר node לראות נתיבים מקבילים</div>}
            {selected && (
              <>
                ...
                {lastEvent && (
                  <div style={{ padding: "7px 10px", borderRadius: 6, border: "1px solid #34d39944", background: "#34d39908", marginBottom: 10 }}>
                    <div style={{ fontSize: 12, color: "#34d399", letterSpacing: 1, marginBottom: 3 }}>✓ DYNAMICS_TRANSITION</div>
                    <div style={{ fontSize: 12, color: "#caf0f8" }}>{lastEvent.event.message}</div>
                    <div style={{ display: "flex", gap: 8, marginTop: 4, fontSize: 12, color: "#8bb8cc" }}>
                ...
                {propagation && (
                  <div style={{ padding: "8px 10px", borderRadius: 6, border: "1px solid #38bdf844", background: "#38bdf808", marginBottom: 10 }}>
                    <div style={{ fontSize: 12, color: "#38bdf8", letterSpacing: 1, marginBottom: 6 }}>
                      ◎ SYSTEM_PROPAGATION
                    </div>
                    ...
                    {propagation.affected.length > 0 && (
                      <div style={{ marginBottom: 6 }}>
                        <div style={{ fontSize: 12, color: "#1e4060", marginBottom: 4 }}>nodes מושפעים:</div>
                        {propagation.affected.map((e, i) => (
                          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                            ...
                            <span style={{ fontSize: 12, color: "#caf0f8", flex: 1 }}>{e.node.name}</span>
                            <span style={{ fontSize: 12, color: "#34d399" }}>+{e.trustDelta}</span>
                            <span style={{ fontSize: 12, color: "#1e4060" }}>{e.strength}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {/* Pulse delta */}
                    <div style={{ display: "flex", gap: 8, fontSize: 12, color: "#8bb8cc", marginBottom: 6, flexWrap: "wrap" }}>
                      {[
                    ...
                    {Object.keys(propagation.flowDeltas).length > 0 && (
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {Object.entries(propagation.flowDeltas).map(([k, v]) => (
                          <span key={k} style={{ fontSize: 12, padding: "1px 5px", borderRadius: 3, background: "#34d39922", color: "#34d399" }}>
                            {k} +{v}%
                          </span>
                        ))}
                      </div>
                  ...
                  return (
                    <div key={i} style={S.card(col)}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                        <span style={{ fontSize: 12, color: col, fontWeight: 700 }}>{t.label}</span>
                        <span style={{ fontSize: 12, color: "#1e4060" }}>+{t.trustDelta} · {t.connections} קשרים</span>
                      </div>
                      <div style={{ fontSize: 12, color: "#caf0f8", marginBottom: 4 }}>{t.action}</div>
        ...
        {tab === "futures" && (
          <div>
            {!selected && <div style={{ fontSize: 12, color: "#1e4060", textAlign: "center", padding: 16 }}>בחר node לראות עתידות חלופיות</div>}
            {selected && comparison && (
              <>
                <div style={{ fontSize: 12, color: "#8bb8cc", marginBottom: 10, lineHeight: 1.5 }}>{comparison.insight}</div>
                ...
                {[
                  ...
                  <div key={label} style={S.card(color)}>
                    <div style={{ fontSize: 12, color, letterSpacing: 1, fontWeight: 700, marginBottom: 4 }}>{label}: {fut.label}</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                      <div>
                        <div style={{ fontSize: 12, color: "#1e4060" }}>שבוע 1</div>
                        <div style={{ fontSize: 13, color, fontWeight: 700 }}>+{fut.week1Trust} trust</div>
                        <div style={{ fontSize: 12, color: "#8bb8cc" }}>{fut.week1Conns} קשרים</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 12, color: "#1e4060" }}>חודש 1</div>
                        <div style={{ fontSize: 13, color, fontWeight: 700 }}>+{fut.month1Trust} trust</div>
                        <div style={{ fontSize: 12, color: "#8bb8cc" }}>{fut.month1Opps} הזדמנויות</div>
                      </div>
                    </div>
                    <div style={{ fontSize: 12, color: "#1e4060", marginTop: 4 }}>תפקיד: {fut.description}</div>
                  </div>
                ))}
              </>
            )}
        ...
        {tab === "longterm" && (
          <div>
            {!selected && <div style={{ fontSize: 12, color: "#1e4060", textAlign: "center", padding: 16 }}>בחר node לראות השפעה ארוכת טווח</div>}
            {selected && longTerm && (
              <>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginBottom: 10 }}>
                  <div style={{ fontSize: 12, color: fc, fontWeight: 700, textAlign: "center", padding: "3px 0" }}>
                    {FORCE_LABEL[longTerm.node.dominantForce]} (נוכחי)
                  </div>
                  <div style={{ fontSize: 12, color: FORCE_COLOR[longTerm.altForce], fontWeight: 700, textAlign: "center", padding: "3px 0" }}>
                ...
                {(["1d","1w","1m","1y"] as const).map((p, i) => {
                  ...
                  return (
                    <div key={p} style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr", gap: 4, marginBottom: 6, alignItems: "center" }}>
                      <div style={{ fontSize: 12, color: "#1e4060", fontWeight: 600 }}>{curr.label}</div>
                      <ProjectionCell p={curr} color={fc} />
                      <ProjectionCell p={alt}  color={FORCE_COLOR[longTerm.altForce]} />
                    </div>
                  );
                })}
                <div style={{ fontSize: 12, color: "#1e4060", marginTop: 8, lineHeight: 1.5 }}>
        ...
        {tab === "tensions" && (
          <div>
            ...
            {tensions.length === 0
              ? <div style={{ fontSize: 12, color: "#1e4060" }}>לא מספיק נתונים</div>
              : tensions.map((t, i) => {
                const col = TENSION_COLOR[t.level];
                return (
                  <div key={i} style={S.card(col)}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                      <div style={{ fontSize: 13, color: "#caf0f8" }}>
                      ...
                      <span style={{ fontSize: 12, padding: "2px 7px", borderRadius: 3, background: col + "22", color: col, fontWeight: 700 }}>
                        {TENSION_LABEL[t.level]}
                      </span>
                    </div>
                    <div style={{ fontSize: 12, color: "#8bb8cc", marginBottom: 3 }}>{t.count} nodes · {t.risk}</div>
                    <div style={{ fontSize: 12, color: "#34d399" }}>✦ {t.growth}</div>
                  </div>
                );
              })
            }
              ...
              : flows.map((f, i) => {
                ...
                return (
                  <div key={i} style={S.card(tCol)}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                      <div style={{ fontSize: 12, color: "#caf0f8", fontWeight: 600 }}>{f.label}</div>
                      <span style={{ fontSize: 13, color: tCol, fontWeight: 700 }}>{FLOW_TREND_ICON[f.trend]}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 5 }}>
                      <span style={{ fontSize: 12, padding: "1px 6px", borderRadius: 8, background: FORCE_COLOR[f.fromForce] + "22", color: FORCE_COLOR[f.fromForce] }}>
                        {FORCE_LABEL[f.fromForce]}
                      </span>
                      <span style={{ fontSize: 12, color: tCol }}>→</span>
                      <span style={{ fontSize: 12, padding: "1px 6px", borderRadius: 8, background: FORCE_COLOR[f.toForce] + "22", color: FORCE_COLOR[f.toForce] }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#1e4060", marginTop: 3 }}>
...
function ProjectionCell({ p, color }: { p: TimeProjection; color: string }) {
  return (
    <div style={{ padding: "5px 7px", borderRadius: 4, border: `1px solid ${color}22`, background: color + "08" }}>
      <div style={{ fontSize: 12, color, fontWeight: 700 }}>+{p.trustDelta}</div>
      <div style={{ fontSize: 12, color: "#1e4060" }}>{p.connections} קשרים</div>
      <div style={{ fontSize: 12, color: "#1e4060" }}>{p.opportunities} הזד'</div>
    </div>
  );
}
